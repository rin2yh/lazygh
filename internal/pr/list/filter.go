...
func FilterPanelLines(filter PRFilterMask, cursor int) ([]string, int) {
...
func buildFilterContent(filter PRFilterMask, cursor int) ([]string, int) {
	lines := []string{""}
	maxW := 0
	for i, opt := range PRFilterOptions {
		check := "[ ]"
		if filter.Has(opt) {
			check = "[x]"
		}
